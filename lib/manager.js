...
export class WorktreeManager {
	...
	async resolveRepoRoot(cwd, signal) {
		...
		return path.resolve(path.dirname(result.stdout));
	}
	...
	async findForCwd(cwd, signal) {
		...
		while (!visited.has(dir)) {
			visited.add(dir);
			const manifestFile = path.join(dir, this.dirName, "manifest.json");
			if (await exists(manifestFile)) {
				const manifest = await this.loadManifest(dir, signal);
				for (const entry of manifest?.worktrees ?? []) {
					if (isWithin(entry.path, cwd)) return { repoRoot: dir, worktree: entry };
				}
				return undefined;
			}
